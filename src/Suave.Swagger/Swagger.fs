...
module Swagger =
  ...
  type FormatParser =
    ...
    member x.Acc(s : string) = x.Buffer := !x.Buffer @ (s.ToCharArray() |> Seq.toList)
    member x.Acc(c : char) = x.Buffer := !x.Buffer @ [ c ]
    member private x.Finished() = !x.Position >= x.Format.Length
    ...
    member x.PreviewNext() =
      if !x.Position >= x.Format.Length - 1 then None
    ...
    member x.Parse() =
      while x.Finished() |> not do
        ...
        | '%' :: 'i' :: _ | '%' :: 'u' :: _ | '%' :: 'd' :: _ -> x.Push [ Parsed IntPart ]
        | '%' :: 'c' :: _ -> x.Push [ Parsed StringPart ]
        | '%' :: 's' :: _ -> x.Push [ Parsed StringPart ]
        | '%' :: 'e' :: _ | '%' :: 'E' :: _ | '%' :: 'f' :: _ | '%' :: 'F' :: _ | '%' :: 'g' :: _ | '%' :: 'G' :: _ -> 
          x.Push [ Parsed DecimalPart ]
        | '%' :: 'x' :: _ | '%' :: 'X' :: _ -> x.Push [ Parsed HexaPart ]
        | _ :: _ -> 
          let n = x.PreviewNext()
          match n with
      ...
      if !x.Buffer
         |> Seq.isEmpty
         |> not
      then x.Push [ Constant(x.StringBuffer 0) ]
      !x.Parts
  ...
  and PathDefinition =
    { Summary : string
      ...
      Responses : IDictionary<int, ResponseDoc> }
    member __.ShouldSerializeParameters() = __.Parameters.Length > 0
  ...
  and ApiDescriptionConverter() =
    ...
    override __.WriteJson(writer : JsonWriter, value : obj, _ : JsonSerializer) =
      ...
      if not (d.Contact = Contact.Empty) then writer.WriteProperty "contact" d.Contact
      if not (d.License = LicenseInfos.Empty) then writer.WriteProperty "license" d.License
      writer.WriteEndObject()
      writer.Flush()
    
    override __.ReadJson(_ : JsonReader, _ : Type, _ : obj, _ : JsonSerializer) = unbox ""
    override __.CanConvert(objectType : Type) = objectType = typeof<ApiDescription>
  ...
  and ResponseDocConverter() =
    ...
    override __.ReadJson(_ : JsonReader, _ : Type, _ : obj, _ : JsonSerializer) = unbox ""
    override __.CanConvert(objectType : Type) = objectType = typeof<ResponseDoc>
  ...
  and PropertyDefinitionConverter() =
    ...
    override __.ReadJson(_ : JsonReader, _ : Type, _ : obj, _ : JsonSerializer) = unbox ""
    override __.CanConvert(objectType : Type) = objectType = typeof<PropertyDefinition>
  ...
  and ParamDefinitionConverter() =
    ...
    override __.ReadJson(_ : JsonReader, _ : Type, _ : obj, _ : JsonSerializer) = unbox ""
    override __.CanConvert(objectType : Type) = objectType = typeof<ParamDefinition>
  ...
  and ObjectDefinitionConverter() =
    ...
    override __.WriteJson(writer : JsonWriter, value : obj, _ : JsonSerializer) =
      ...
      writer.WritePropertyName "propertiesf"
      writer.WriteStartObject()
    ...
    override __.ReadJson(_ : JsonReader, _ : Type, _ : obj, _ : JsonSerializer) = unbox ""
    override __.CanConvert(objectType : Type) = objectType = typeof<ObjectDefinition>
  
  and PropertyDefinition =
    | Primitive of Type : string * Format : string
    | Ref of ObjectDefinition
    | ArrayPrimitive of Type : string * Format : string
    | ArrayRef of ObjectDefinition
    ...
    member __.ToJObject() : JObject =
      ...
      | Ref ref -> v.Add("$ref", JToken.FromObject <| sprintf "#/definitions/%s" ref.Id)
      | ArrayPrimitive(t, f) -> 
        let items = JObject()
        items.Add("type", JToken.FromObject t)
        items.Add("format", JToken.FromObject f)
        v.Add("type", JToken.FromObject "array")
        v.Add("items", items)
      | ArrayRef ref -> 
        let items = JObject()
        items.Add("$ref", JToken.FromObject <| sprintf "#/definitions/%s" ref.Id)
        v.Add("type", JToken.FromObject "array")
        v.Add("items", items)
      v
    
    member __.ToJson() : string = __.ToJObject().ToString()
  ...
  and ParamDefinition =
    ...
    member __.ToJObject() : JObject =
      ...
      | Some t -> 
        match t with
        | Primitive(t, _) -> v.Add("type", JToken.FromObject t)
        | Ref _ -> v.Add("schema", t.ToJObject())
        | ArrayPrimitive _ -> failwith "TODO"
        | ArrayRef _ -> failwith "TODO"
      | None -> ()
      v
  ...
  type Type with
    member this.IsSwaggerPrimitive = TypeHelpers.typeFormatsNames.ContainsKey this
    
    member this.FormatAndName =
      match this with
      | _ when TypeHelpers.typeFormatsNames.ContainsKey this -> Some(TypeHelpers.typeFormatsNames.Item this)
      | _ when this.IsPrimitive -> Some(TypeHelpers.typeFormatsNames.Item(typeof<string>))
      | _ -> None
    
    member this.Describes() : ObjectDefinition =
      let optionalType (t : Type) =
        if (not t.IsGenericType) || t.GetGenericTypeDefinition() <> typedefof<Option<_>> then None
        else 
      ...
      let rec describe (t : Type) =
        let descProp (tp : Type) name =
          ...
          | Some(ty, na) -> Some(name, Primitive(ty, na))
          | None -> 
            if tp.IsArray then 
              let ti = tp.GetElementType()
              match ti.FormatAndName with
              | Some x -> Some(name, ArrayPrimitive(x))
              | None -> Some(name, ArrayRef(describe ti))
            else if t = tp then None
            else Some(name, Ref(describe tp))
  ...
  let combineUrls (u1 : string) (u2 : string) =
    let sp =
      if u2.StartsWith "/" then u2.Substring 1
      else u2
    u1 + sp
  
  let streamWp (stream : Stream) : WebPart =
    fun ctx -> 
      let write (conn, _ : HttpResult) : SocketOp<Connection> =
        socket { 
      ...
      { ctx with response =
                   { ctx.response with status = { ctx.response.status with code = 200 }
            ...
            if p = "index.html" then 
              ...
              { ctx with response =
                           { ctx.response with status = { ctx.response.status with code = 200 }
                                               ...
                                               headers = headers } }
              |> succeed
          | None -> ctx |> NOT_FOUND "Ressource not found"
        
        streamZipContent()
    pathStarts swPath >=> wp
  
  let removeDefaultResponseDoc = Seq.filter (fun (code : int, r : ResponseDoc) -> code <> 200 && not (r.IsDefault()))
  ...
  type DocBuildState =
    ...
    member this.Combine(other : DocBuildState) =
      if this.Id <> other.Id then 
        { this with Routes = ((this.Routes @ other.Routes) @ [ other.Current ] |> List.rev)
        ...
        let d =
          if o.Description = "" then state.Description
          else o.Description
        
        let t =
          if o.Template = "" then state.Template
          else o.Template
        ...
        { this with Routes = (this.Routes @ other.Routes)
                    Current =
                      { this.Current with Description =
                                            { this.Current.Description with Description = d
                                                                            Template = t
                                                                            Params = p
                                                                            Responses = rs
                                                                            Consumes =
                                                                              o.Consumes @ state.Consumes 
                                                                              |> List.distinct
                                                                            Produces =
                                                                              o.Produces @ state.Produces 
                                                                              |> List.distinct
                                                                            Tags = o.Tags @ state.Tags |> List.distinct } }
                    Models = m }
    
    member this.Describes(f : ApiDescription -> ApiDescription) = { this with Description = (f this.Description) }
    ...
    member __.Documentation =
               ...
               seq { 
                 for p in g do
                   ...
                   let rs =
                     p.Responses
                     |> fun responses -> 
                       if responses.Count > 1 && responses
                                                 |> toTuples
                                                 |> Seq.exists (function 
                                                      | (200, d) when d.IsDefault() -> true
                                                      | _ -> false)
                       then 
                         responses
                         |> toTuples
                         ...
                         |> Seq.toList
                         |> dict
                       elif responses.Count <= 0 then dict [ (200, ResponseDoc.Default) ]
                       else responses
      ...
      for d in List.ofSeq definitions.Values do
        for p in d.Properties do
          match p.Value with
          | Ref r -> 
            if not <| definitions.ContainsKey r.Id then definitions.Add(r.Id, r)
          | ArrayRef r ->
            if not <| definitions.ContainsKey r.Id then definitions.Add(r.Id, r)
          | _ -> ()
    ...
    member __.App =
      let swaggerWebPart =
        path __.SwaggerJsonPath >=> OK(__.Documentation.ToJson()) 
        >=> Writers.setMimeType "application/json; charset=utf-8" 
        >=> Writers.addHeader "Access-Control-Allow-Origin" "*"
      let oldSwaggerUiPart = pathStarts "/swagger/v2/ui" >=> Redirection.redirect __.SwaggerUiPath
      let uiWebpart = swaggerUiWebPart __.SwaggerUiPath __.SwaggerJsonPath
      choose 
        ((__.Routes |> List.map (fun r -> r.WebPart)) 
         @ [ oldSwaggerUiPart; swaggerWebPart; uiWebpart; __.Current.WebPart ])
  
  type SwaggerBuilder() =
    member __.Yield(w : DocBuildState) : DocBuildState = w
    
    member __.For(w : WebPart, func : DocBuildState -> DocBuildState) : DocBuildState =
      let b = DocBuildState.Of w
      func b
    
    member __.For(b : DocBuildState, func : DocBuildState -> DocBuildState) : DocBuildState = func b
    member __.Combine((b1, b2) : DocBuildState * DocBuildState) : DocBuildState = b1.Combine b2
    member __.Delay(func : unit -> DocBuildState) : DocBuildState = func()
