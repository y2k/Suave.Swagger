...
module FunnyDsl =
  ...
  let consumes (mimetype : string) (route : DocBuildState) =
    { route with Current =
                   { route.Current with Description =
                                          { route.Current.Description with Consumes =
                                                                             mimetype 
                                                                             :: route.Current.Description.Consumes } } }
  let produces (mimetype : string) (route : DocBuildState) =
    { route with Current =
                   { route.Current with Description =
                                          { route.Current.Description with Produces =
                                                                             mimetype 
                                                                             :: route.Current.Description.Produces } } }
  let tag (tag : string) (route : DocBuildState) =
    { route with Current =
                   { route.Current with Description =
                                          { route.Current.Description with Tags = tag :: route.Current.Description.Tags } } }
  ...
  let addResponse (statusCode : int) (desc : string) (modelType : Type option) (route : DocBuildState) =
    ...
    { s with Current = { s.Current with Description = { s.Current.Description with Responses = rr } } }
  ...
  let simpleUrl (p : string) = (DocBuildState.Of <| path p).Documents(fun doc -> { doc with Template = p })
  ...
  let pathTemplate (pf : PrintfFormat<_, _, _, _, 't>) (names : string list) f =
    ...
    let getName i =
      if i < names.Length then names.Item i
      else sprintf "param%d" i
    
    let _, tmpl =
      parts
      |> List.fold (fun ((i, acc) : int * string) (p : FormatPart) -> 
           match p with
           | Constant c -> i, acc + c
           | Parsed _ -> (i + 1), sprintf "%s{%s}" acc (getName i)) (0, "")
    ...
    (match ty with
     | _ when ty.IsPrimitive || TypeHelpers.typeFormatsNames.ContainsKey ty -> 
       ...
       let p =
         { (ParamDescriptor.Named name) with Type = (Some ty)
                                             In = Path }
     ...
     | _ -> 
       ...
       |> Seq.mapi (fun i pt -> 
            ...
            let t = Some pt
            { (ParamDescriptor.Named name) with Type = t
                                                In = Path })
       |> Seq.rev
       |> Seq.fold (fun (state : DocBuildState) p -> state.Documents(fun d -> { d with Params = (p :: d.Params) })) doc)
    |> fun d -> d.Documents(fun doc -> { doc with Template = tmpl })
  
  let urlFormat (pf : PrintfFormat<_, _, _, _, 't>) f = pathTemplate pf [] f
  let thenReturns w (d : DocBuildState) = { d with Current = { d.Current with WebPart = (d.Current.WebPart >=> w) } }
